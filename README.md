...
## 项目结构
- `index.html`：入口页面，仅承担表单与结果的容器职责。
- `styles.css`：全局样式定义。
- `src/domain/`：核心财务模型与计算器（如复利推演、个税处理）。
- `src/data/`：表单数据解析与标准化，产出统一的财务画像。
- `src/analytics/`：生成年度投射结果与结论所需的数据。
- `src/presentation/`：界面交互逻辑、视图切换与结果渲染。
